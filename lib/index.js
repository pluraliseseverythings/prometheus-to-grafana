...
function makeGrafana(metrics, title) {
    ...
    function makePanel() {
        var metric_here = this_row[metric_i];
        left -= 1;
        var fmn = metric_here['full_metric_name'];
        var targets = [fmn];
        if (metric_here['has_quantile']) {
            targets = [fmn + '{quantile="0.5"}', fmn + '{quantile="0.99"}']
        }
        return new Panels.Graph({
            title: metric_here['metric_name'],
            span: 4,
            targets: targets,
            legend: {
                avg: false,
    ...
    for (var row in rows) {
        ...
        var left = this_row.length;
        for (var metric_i in this_row) {
            var panel = makePanel();
